...
int main(int argc, char const *argv[])
{
    ...
    if(inet_pton(AF_INET, "156.56.104.84", &serv_addr.sin_addr)<=0) 
    {
    ...
    FILE *fp;
    char *filename = "1g.img";
    
    fp = fopen(filename, "r");

    int mb = 0;
    while(fgets(buffer, 1024 * 1024, fp) != NULL) {
      encrypt(buffer, encrypted, BUFSIZE, iv, key);
      send(sock , encrypted , 1024 * 1024 , 0 );
      mb++;
